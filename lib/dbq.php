...
function td_and_modal($week) {
	...
	foreach ($week as $dayarr => $value) {
		...
		foreach ($value as $my) {
			...
			$tr_open = '<tr>';
			$tr_close = '</tr>';
			$table_td = '<div><td class="btn btn-link" data-toggle="modal" data-target="#' . 'cell' . $my["id"] . '"' . ' style="display: block-inline; height: 100%;">' .
				$my["intensity"] .
				'</button><div><img src=' . '"' . $my["image"] . '"' . 'alt="..." class="rounded-circle" style="width: 75px; height: 75px;"><div><p>' . $my["adress"] . '</p></div><div><p>' . $my["start_time"] . '</p></div><div><p>' . $my["capacity"] . '</p></div></td></div>';
